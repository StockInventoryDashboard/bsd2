...
document.addEventListener('DOMContentLoaded', () => {
   ...
   document.getElementById('addProductBtn').addEventListener('click', () => {
        if (db.capital.initial <= 0 && currentUser.role === 'admin') {
            alert('Please set the initial capital before adding products. Go to Capital Tracker to set it.');
            showPage('capitalTrackerPage'); // Direct admin to capital tracker
            return;
        } else if (db.capital.initial <= 0 && currentUser.role !== 'admin') {
             alert('Initial capital has not been set by an administrator. Products cannot be added yet.');
             return;
        }
